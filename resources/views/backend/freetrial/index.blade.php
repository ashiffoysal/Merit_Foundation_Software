...
<div id="modal-view-message" class="modal-backdrop" style="display:none" onclick="closeModalOutside(event,'modal-view-message')">
    <div class="modal-box" style="max-width:560px">
        <div class="modal-header-custom">
            <div class="card-title"><i class="fas fa-envelope-open-text"></i> Free Trial Detail</div>
        ...
        <div class="modal-body-custom" id="messageDetailBody">
            <div class="msg-detail-row"><span class="msg-label">Parent</span><span id="md-parent" class="msg-val"></span></div>
            <div class="msg-detail-row"><span class="msg-label">Child</span><span id="md-child" class="msg-val"></span></div>
            <div class="msg-detail-row"><span class="msg-label">Age</span><span id="md-age" class="msg-val"></span></div>
            <div class="msg-detail-row"><span class="msg-label">Level</span><span id="md-level" class="msg-val"></span></div>
            <div class="msg-detail-row"><span class="msg-label">Tutor Pref</span><span id="md-tutor" class="msg-val"></span></div>
            <div class="msg-detail-row"><span class="msg-label">Country</span><span id="md-country" class="msg-val"></span></div>
            <div class="msg-detail-row"><span class="msg-label">Email</span><span id="md-email" class="msg-val"></span></div>
            <div class="msg-detail-row"><span class="msg-label">WhatsApp</span><span id="md-whatsapp" class="msg-val"></span></div>
            <div class="msg-detail-row"><span class="msg-label">Pref. Time</span><span id="md-time" class="msg-val"></span></div>
...
<div class="container">
    <div class="page-header d-flex align-items-start justify-content-between">
        <div>
            <h1>Free Trial Enquiries</h1>
            <p>All incoming free trial requests</p>
        </div>
        <div class="d-flex gap-2">
    ...
    <div class="card">
        <div class="card-header-custom">
            <div class="card-title">
                <i class="fas fa-comments"></i> All Free Trial
                ...
                <table class="data-table" id="messagesTable">
                    <thead>
                        <tr>
                            ...
                            <th class="sortable" data-col="parent_name" onclick="sortBy('parent_name')">
                                Name <i class="fas fa-sort sort-ico" id="sort-parent_name"></i>
                            </th>
                            <th class="sortable" data-col="email" onclick="sortBy('email')">
                                Email <i class="fas fa-sort sort-ico" id="sort-email"></i>
                            </th>
                            <th>WhatsApp</th>
                            <th class="sortable" data-col="current_level" onclick="sortBy('current_level')">
                                Level <i class="fas fa-sort sort-ico" id="sort-current_level"></i>
                            </th>
                            <th>Country</th>
...
function loadTable(page = 1) {
    ...
    fetch(`{{ route('admin.free.getindex') }}?${params}`, {
...
function renderTable(rows) {
    if (!rows.length) {
        document.getElementById('tableBody').innerHTML =
            '<tr><td colspan="9" class="tbl-empty"><i class="fas fa-inbox"></i> No records found.</td></tr>';
        return;
    }
    const statusMap = {
    ...
    document.getElementById('tableBody').innerHTML = rows.map(r => `
        <tr id="row-${r.id}">
            ...
            <td>
                <div style="font-weight:600;font-size:.82rem">${esc(r.parent_name)}</div>
                <div style="font-size:.72rem;color:var(--muted)">Child: ${esc(r.child_name)}${r.child_age ? ' ('+esc(r.child_age)+')' : ''}</div>
            </td>
            <td><span style="font-size:.8rem">${esc(r.email)}</span></td>
            <td><span style="font-size:.78rem">${esc(r.whatsapp||'—')}</span></td>
            <td><span style="font-size:.78rem">${esc(r.current_level||'—')}</span></td>
            <td><span style="font-size:.78rem">${esc(r.country||'—')}</span></td>
...
function renderPagination(meta) {
    ...
    document.getElementById('paginationInfo').textContent =
        total ? `Showing ${from}–${to} of ${total} records` : 'No results';
...
function viewMessage(id) {
    ...
    .then(d => {
        const m = d.message;
        if (!m) throw new Error('Record not found in response');

        document.getElementById('md-parent').textContent   = m.parent_name ?? '—';
        document.getElementById('md-child').textContent    = m.child_name ?? '—';
        document.getElementById('md-age').textContent      = m.child_age ?? '—';
        document.getElementById('md-level').textContent    = m.current_level ?? '—';
        document.getElementById('md-tutor').textContent    = m.tutor_gender ?? '—';
        document.getElementById('md-country').textContent  = m.country ?? '—';
        document.getElementById('md-email').textContent    = m.email ?? '—';
        document.getElementById('md-whatsapp').textContent = m.whatsapp ?? '—';
...
function markStatus(id, status, reload = true) {
    ...
    .then(d => {
        if (d.success) {
            if (reload) { closeModal('view-message'); loadTable(currentPage); }
...
function bulkAction(action) {
    ...
    if (action === 'delete' && !confirm(`Delete ${ids.length} record(s)?`)) return;
