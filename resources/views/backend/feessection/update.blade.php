...
<div class="container">
    <div class="page-header d-flex align-items-center justify-content-between">
        <div>
            <h1>Edit Plan</h1>
            <p>Update subscription plan: <strong>{{ $edit->name }}</strong></p>
        </div>
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            <form id="plan-form" method="POST" novalidate>
                                ...
                                <div class="f-group">
                                    ...
                                    <select name="category_id" id="category_id" class="f-input">
                                        ...
                                        @foreach($categories as $cat)
                                            <option value="{{ $cat->id }}"
                                                {{ $edit->category_id == $cat->id ? 'selected' : '' }}>
                                                {{ $cat->category_name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-4">
                                <div class="f-group">
                                    ...
                                    <input type="text" name="button_text" id="button_text"
                                           class="f-input" placeholder="Choose Plan"
                                           value="{{ $edit->button_text ?? 'Choose Plan' }}">
                            ...
                            <div class="col-md-12">
                                <div class="f-group">
                                    ...
                                    <textarea name="description" id="description" rows="3"
                                              class="f-input" placeholder="Full details about the plan…">{{ $edit->description }}</textarea>
                            ...
                            <div class="col-md-6">
                                <div class="f-group">
                                    ...
                                    <select name="duration" id="duration" class="f-input">
                                        ...
                                        <option value="30_minutes" {{ $edit->duration === '30_minutes' ? 'selected' : '' }}>30 Minutes</option>
                                        <option value="1_hour"     {{ $edit->duration === '1_hour'     ? 'selected' : '' }}>1 Hour</option>
                                    </select>
                            ...
                            <div class="col-md-4">
                                <div class="f-group">
                                    ...
                                    <select name="currency" id="currency" class="f-input">
                                        <option value="GBP" {{ $edit->currency === 'GBP' ? 'selected' : '' }}>GBP — £</option>
                                        <option value="USD" {{ $edit->currency === 'USD' ? 'selected' : '' }}>USD — $</option>
                                        <option value="EUR" {{ $edit->currency === 'EUR' ? 'selected' : '' }}>EUR — €</option>
                                        <option value="BDT" {{ $edit->currency === 'BDT' ? 'selected' : '' }}>BDT — ৳</option>
                                    </select>
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body-custom">
                        ...
                        <div id="features-list" class="d-flex flex-column gap-2 mb-3">
                            {{-- Populated by JS from $edit->features --}}
                        </div>
                ...
                <div class="card mb-4">
                    <div class="card-header-custom">
                        <div class="card-title"><i class="fas fa-clock-rotate-left"></i> Record Info</div>
                    </div>
                    <div class="card-body-custom">
                        <div class="row g-3" style="font-size:.85rem;color:#6c757d">
                            <div class="col-md-6">
                                <span class="f-label d-block mb-1">Created</span>
                                {{ $edit->created_at->format('d M Y, h:i A') }}
                            </div>
                            <div class="col-md-6">
                                <span class="f-label d-block mb-1">Last Updated</span>
                                {{ $edit->updated_at->format('d M Y, h:i A') }}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="d-flex gap-2 mb-5">
                    <button type="submit" class="btn-gold-sm" id="submit-btn">
                        <span id="submit-text"><i class="fas fa-save"></i> Update Plan</span>
...
{{-- Reuse identical styles from create --}}
<style>
.invalid-feedback-custom { color:#dc3545;font-size:.8rem;margin-top:.3rem;display:none; }
.invalid-feedback-custom.visible { display:block; }
.f-input.is-invalid { border-color:#dc3545 !important; }
.input-prefix-wrap { position:relative; }
.input-prefix { position:absolute;left:.75rem;top:50%;transform:translateY(-50%);color:#888;font-size:.9rem;pointer-events:none; }
...
.feature-row .remove-feature { background:none;border:none;color:#dc3545;font-size:1rem;cursor:pointer;padding:.25rem .5rem;border-radius:4px;transition:background .2s; }
.feature-row .remove-feature:hover { background:#fdf0f0; }
.plan-preview-card { background:linear-gradient(135deg,#1a1a2e 0%,#16213e 60%,#0f3460 100%);border-radius:16px;padding:1.5rem 1.25rem;color:#fff;position:relative;overflow:hidden; }
.plan-preview-card::before { content:'';position:absolute;width:120px;height:120px;background:rgba(255,215,0,.08);border-radius:50%;top:-40px;right:-30px; }
.plan-preview-badge { display:inline-block;background:#ffd700;color:#1a1a2e;font-size:.65rem;font-weight:700;letter-spacing:.08em;padding:.2rem .6rem;border-radius:20px;margin-bottom:.6rem; }
.plan-preview-name { font-size:1.3rem;font-weight:700;margin-bottom:.2rem; }
.plan-preview-subtitle { font-size:.78rem;opacity:.65;margin-bottom:.8rem;min-height:1rem; }
.plan-preview-price { display:flex;align-items:baseline;gap:.15rem;margin-bottom:.3rem; }
...
.plan-preview-meta { font-size:.75rem;opacity:.55;margin-bottom:1rem; }
.plan-preview-features { list-style:none;padding:0;margin:0 0 1.2rem;display:flex;flex-direction:column;gap:.35rem; }
.plan-preview-features li { font-size:.8rem;display:flex;align-items:center;gap:.45rem; }
.plan-preview-features li::before { content:'✓';color:#ffd700;font-weight:700;font-size:.85rem; }
.plan-preview-btn { width:100%;background:#ffd700;color:#1a1a2e;border:none;border-radius:8px;padding:.55rem 1rem;font-weight:700;font-size:.85rem;cursor:default; }
.tips-list { list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:.6rem;font-size:.82rem;color:#555; }
</style>
...
(function () {
    ...
    // Existing features from model (safely parsed)
    const existingFeatures = (function () {
        try {
            const raw = @json(is_array($edit->features) ? $edit->features : []);
    ...
    function clearErrors() {
        ...
        document.querySelectorAll('.f-input.is-invalid').forEach(el => el.classList.remove('is-invalid'));
    }
    ...
    // Seed existing features (or one blank row if none)
    if (existingFeatures.length) {
        existingFeatures.forEach(f => addFeatureRow(f));
    } else {
        addFeatureRow();
    }
    ...
    function updatePreview() {
        ...
        const sym = currencySymbols[val('currency') || 'GBP'] || '£';
    ...
    $('plan-form').addEventListener('submit', async function (e) {
        ...
        try {
            const res  = await fetch('{{ route("admin.plans.update", $edit->id) }}', {
            ...
            if (res.ok && data.success) {
                showAlert('success', data.message ?? 'Plan updated successfully!');
                if (data.redirect) {
